...
通过 **Widget** 组件开启一个或多个对齐参考后，节点的位置（position）和尺寸（width，height）属性可能会被限制，不能通过 API 或动画系统自由修改。如果需要在运行时修改对齐节点的位置或尺寸，请参考[Widget 组件参考：对节点位置、尺寸的限制](../components/widget.html#--5)相关内容。

---

继续前往 [文字排版](label-layout.md) 说明文档。
