...
在通过 [访问节点和组件](access-node-component.md) 介绍的方法获取到节点或组件实例后，这篇文章将会介绍通过节点和组件实例可以通过哪些常用接口实现我们需要的种种效果和操作。这一篇也可以认为是 [Node](__APIDOC__/zh/classes/scene_graph.node-1.html) 和 [Component](__APIDOC__/zh/classes/component.component-1.html) 类的 API 阅读指南，可以配合 API 一起学习理解。
