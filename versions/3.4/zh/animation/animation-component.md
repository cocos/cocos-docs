...
动画组件通过 [play()](%__APIDOC__%/zh/#/docs/3.4/zh/animation/Class/Animation?id=play) 控制指定动画的播放，例如：
...
使用 `play` 接口播放一个动画时，如果此时还有其他的动画正在播放，则会立即停止其他动画的播放。这种切换是非常突兀的，在某些情况下，我们希望这种切换是“淡入淡出”的效果，那么便可以使用 [crossFade()](%__APIDOC__%/zh/#/docs/3.4/zh/animation/Class/Animation?id=crossfade)，在指定的周期内平滑地完成切换。例如：
...
关于动画组件更多相关的控制接口，详情请参考 [类 `Animation`](%__APIDOC__%/zh/#/docs/3.4/zh/animation/Class/Animation)。
...
更多内容请参考 [Animation.EventType](%__APIDOC__%/zh/#/docs/3.4/zh/animation/Class/Animation?id=eventtype)。
