...
> **注意**：节点上的四元数 [rotation](__APIDOC__/zh/#/docs/3.3/zh/scene-graph/Class/Node?id=rotation) 属性，表示的是绕任意轴旋转的角度。而 **属性检查器** 中的 `Rotation` 所对应的属性是欧拉角属性 [EulerAngles](__APIDOC__/zh/#/docs/3.3/zh/scene-graph/Class/Node?id=eulerangles)，在脚本中可通过 `Node.eulerAngles` 获取 `Rotation` 的值。这两个属性可以根据需求分别使用，在使用 API 时请一定要注意它们和编辑器面板属性名的对应区别。
