...
Cocos Creator has supported the material system since v2.x. However, the design of the material system and the built-in Shader API are continually being improved, there may be some adjustments in the upgrade from v2.x to v3.0 and subsequent upgrades. The Effect asset does not currently support automatic upgrades, so please refer to the following documentation to upgrade:
