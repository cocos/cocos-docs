...
In the customizable render pipeline, you can choose to use the engine's built-in render pipeline. The built-in pipeline includes a **forward render pipeline** and a **deferred render pipeline**, and the engine uses the forward render pipeline by default. Please refer to the [Built-in Render Pipeline](builtin-pipeline.md) documentation for details.
