...
1. Create a new 3D object Cube, click the **+** Create button in the top left corner of **Hierachy** panel, then select **Create -> 3D Object -> Cube Cube**.
...
TerrainCollider is a collider generated by a terrain surface, with the same specific shape as the object it is attached to on the Terrain asset.
...
2. If you find a **RigidBody** component, you bind your own **Collider** component to that node, otherwise the **Collider** components in the whole chain will share a single **RigidBody** component, and the node corresponding to the element is the node corresponding to the top-level **Collider** component. **Collider** component.
