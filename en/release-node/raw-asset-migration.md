...
Starting with v1.10, built textures are named with their UUID, which causes you to not be able to directly determine the location of the asset in the project from the filename. Dealing with this in v1.10 is a bit cumbersome, it is recommended to upgrade directly to v2.0, and then you need to make some customizations to the build process, please refer to the example <https://github.com/cocos-creator/demo-process-build-textures>.
