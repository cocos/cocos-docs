...
[Cloud Storage](https://developer.huawei.com/consumer/en/doc/development/AppGallery-connect-Guides/agc-cloudstorage-introduction) is scalable and maintenance-free. It allows you to store high volumes of data such as images, audios, and videos generated by your users securely and economically with direct device access.
...
- [Applying for the Cloud Storage Service](#applying-for-the-cloud-storage-service) by sending E-Mail.
...
- Run the project on a phone, if you can see the file name of the uploaded file output in Logcat, which means the integrate is successful.

## Sample Project

Developer can get a quick taste of the Cloud Storage service with the sample project.

- Please refer to [Upload a file](https://developer.huawei.com/consumer/en/doc/development/AppGallery-connect-Guides/agc-cloudstorage-manage#h1-1591181343625) document, upload a [legal file](#legality-of-files) named `output.json`.
...
Developer need to initialize the cloud storage instance by calling `getInstance` method before using the cloud storage function.

```js
var storageManagement = huawei.agc.storage.storageService.getInstance();
```

If you have multiple cloud storage instances and need to use a non-default cloud storage instance, you need to pass in the **storage instance name**.
...
| Parameter | Description |
| :--- | :--- |
| filePath | Path to which data needs to be written, which must be created in advance. |
