...
| **Packing**            | shadowMap compression, which can be turned on for some devices that do not support floating point textures. This optimises performance, but the texture quality is lower (mutually exclusive with the Linear term, you can only choose one of the two). |
