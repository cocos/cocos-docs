...
| **AlignSpace** | The particle align direction space which includes local space, world space and view space (default). Different space will get different initial particle direction. For example if we use local align space will cause particles of the emitter to use its node's rotation. |
