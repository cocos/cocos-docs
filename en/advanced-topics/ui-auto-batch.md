...
**This document is based on Cocos Creator v2.0.x**

In game development, Draw call is a very important performance indicator that directly affects the overall performance of the game. Draw call is the CPU call graphics API, such as OpenGL, command GPU to draw graphics. One Draw call represents one graphics drawing command. Due to the CPU and GPU rendering state switching consumption caused by Draw call, it is often necessary to reduce the number of Draw call calls by batch merging. The essence of batch rendering is to ensure that the rendering state of consecutive nodes is consistent. The data of consecutive nodes is merged and then submitted once, thereby reducing the number of calls of drawing instructions and reducing the performance consumption caused by graphics API calls, and also avoiding The GPU performs frequent rendering state switching. The rendering state includes: Texture state, Blend state, Stencil state, Depth Test state, and so on.
...
In some cases, the edge of the PNG picture will appear black edge due to the color (0,0,0,0) sampled into the transparent area. Usually, in order to solve the problem, the pre-multiplied state of the picture is set, and setting the Src Blend Factor to ONE, but the Blend state is inconsistent will also interrupt the batch merge.
...
![UI Test Debug](./ui-auto-batch/ui-mask.png)
...
4. WeChat mini-game platform disables the dynamic atlas function by default due to the memory usage of Image. If the game requires less memory, you can open it by cc.dynamicAtlasManager.enabled = true.
