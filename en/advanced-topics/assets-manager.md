...
During the download process, problems with the downloaded file contents may occur due to network or other network libraries. So we provide the user file verification interface, which is called by the hot update assetsManager after the file is downloaded (in the case of user implementation). If returning `true` indicates that the file is OK, returning `false` indicates that there is a problem with the file.
...
The asset version in Manifest is recommended to use md5. You can determine whether the file is normal by calculating the md5 code of the downloaded file in the verify function and comparing it with the md5 of the asset. In addition to md5, the asset object also contains the following properties:

1. path:            The relative path of the server side
2. compressed:      Is it a compressed file
3. size:            File size
4. downloadState:   Download size, including `UNSTARTED`, `DOWNLOADING`, `SUCCESSED`, `UNMARKED`
 
### Error message handling and download retry

On the left side of the flowchart, you can see a number of user messages that can be notified through the event listener of hot update. For details, you can refer to the [Implementation of hot update](https://github.com/cocos-creator/tutorial-hot-update/blob/master/assets/scripts/module/HotUpdate.js#L43) in the example. The flowchart identifies the trigger and cause of all error messages, and you can handle them according to your system design.

The most important thing is that when an exception occurs during the download process, such as failed download, failed decompression and failed verification, the `UPDATE_FAILED` event will be triggered.

At this point, a list of all download failed assets is recorded in the hot update assetsManager, and you can download it again in the following ways:

```js
assetsManager.downloadFailedAssets();
```

When the interface is called, it re-enters the hot update process and downloads only assets that failed downloads before. The entire process is the same as the normal hot update process.

### The necessary of restart

If you want to use hot updated assets, you need to restart the game.

There are two reasons, the first is that the updated script requires a clean JS environment. The second is the scene configuration, which in `AssetsLibrary` needs to be updated to the latest to load the scene and assets properly.

1. Refresh of JS script
    
    Before the hot update, all the scripts in the game have been executed, and all the classes, components, objects have already been stored in JS context. So if you load the script directly after the hot update without restarting the game, the classes and objects of the same name will be overwritten, but the objects created by the old class will also exist. And for the global objects that are directly overwritten, their original state is also reset, causing the new and old versions of the objects to mix together. There is also an additional overhead on memory.

2. Refresh of asset configuration

    In Cocos2d-x/JS, you can use new assets such as textures, fonts and sound effects without restarting the game. But not in Creator, because Creator's assets depend on configuration, scene depend on the list of scene in `settings.js`, and `raw assets` depends on the list of `raw assets` in `settings.js`. If `settings.js` is not reexecuted and re-read by `main.js` and `AssetsLibrary`, the game will not load the hot updated scenes and assets.

Enabling new assets relies on the search path mechanism of the Cocos engine. All files on the Cocos are read by FileUtils, and FileUtils looks for files in the priority order of the search path.
So we add the hot update cache directory to the search path and preposition it so that the assets in the cache directory are searched first. Here is the code example:
...
if (jsb) {
    ...
    // hotUpdateSearchPaths will precede at the beginning of the searchPaths array
...
Note that this code must be placed in `main.js` and executed before it require other script, otherwise it will still load the script in the application package.

## Advanced Topics

The above section describes most of the implementation and usage of the Hot Update AssetsManager, but in some special scenarios, you may need some special tricks to avoid the problems raised by hot updates. But in some specific application scenes, you may need some special techniques to avoid problems caused by hot updates.
...
1. What happens during local cache coverage?
    
    When a local cached version already exists in the user's environment, the hot update assetsManager compares the cached version and the in-app version, and then uses the newer version as the local version. If the remote version is updated at this time, the hot update assetsManager uses a temporary folder to download the remote version during the update process. When the remote version is successfully updated, the contents of the temporary folder are copied to the local cache folder. If there are files of the same name in the local cache folder, they are overridden. And the other files are retained, because these files may still be valid, they just have not been modified in this release. Finally delete the temporary folder.
   
    So, in theory, there's no problem with continuous hot update for minor version.

2. How to clean up the local cache during the game's major release update?

    During the package updates, there are various ways to thoroughly clean up the local hot update cache, such as recording the current game version number, checking if it matches the saved version in `cc.sys.localStorage`, and performing the following cleanup if they don't match:
