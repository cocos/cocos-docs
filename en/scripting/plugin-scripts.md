...
Variable Declaration | The variables defined within script are not exposed to global | Once published, local variables declared within the script that are not in any function are exposed as global variables. If in the editor, then it is the same as the Normal Script. (The local variables won't be exposed as global variables on Wechat, Baidu, Xiaomi, Alipay and ByteDance minigame. To achieve the same effect, you should declare the local variables as properties in global variable `window`)
