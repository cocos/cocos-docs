...
- 统一了项目内资源模板、构建模板、预览模板的文件位置，项目升级后将会自动迁移，请及时提交相关的迁移内容。其中构建、预览模板为了保障插件的兼容性，会在迁移后创建超链接到原地址。在确认项目内没有直接操作这些文件地址后，可以直接删除自动创建的超链接。

![template-migrate](img/template-migrate.png)
