...
- 当前版本：0.5.3_1.2.3.301
    ...
    - 修改 `subscribe` 方法，构造查询条件变更为仅支持等值订阅，即仅使用 `equalTo` 方法构造查询条件。
...
| zoneId | Cloud DB zone 名称，表示一个唯一的数据存储区域 |
| typeName | 对象类型名称 |
| queryId | 自定义查询 ID，用于查询接口。构造查询条件时，当前仅支持等值订阅，即仅支持使用 [equalTo()](https://docs.cocos.com/service/api/classes/huawei.agc.db.agcclouddbzonequery.html#equalto) 方法，且查询条件中最少包含一个字段，最多包含 5 个字段，多个查询条件之间采用 **与** 运算。 |
| subscribeId | 订阅器的 ID |
| result | 返回信息 |
