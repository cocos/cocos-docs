...
- 融合定位：结合 GPS、Wi-Fi和基站位置数据，为应用开发者提供一套简单易用的API，方便开发者快速获取设备位置信息。
...
如果您的应用需要请求设备位置信息，您可以为您的应用申请位置权限，然后调用HMS Core的请求位置更新接口，设置不同的请求参数，根据您的需要指定定位方式，获取持续的位置信息回调。如果您获取位置信息后，想取消位置信息回调，可以调用移除位置更新接口，达到取消回调的目的。
