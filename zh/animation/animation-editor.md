...
3. **动画时间轴与关键帧预览**：用于显示/设置当前节点（及其子节点）添加的事件帧以及所添加的动画属性轨道上所有关键帧的预览。
4. **动画属性轨道列表**：用于显示/设置当前选中的节点在当前编辑的动画剪辑中包含的所有动画属性轨道。
5. **动画属性轨道关键帧预览**：用于显示/设置当前选中节点的各动画属性轨道上的所有关键帧。
...
在 **动画时间轴** 和 **动画属性轨道关键帧预览** 任一区域内滚动鼠标滚轮，即可放大或者缩小时间轴的显示比例。
