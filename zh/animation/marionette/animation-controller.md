...
- **出口**：当状态过渡到 **出口** 后，退出状态机。**入口** 只能作为状态过渡的目标，无法作为过渡的源头。

    **出口仅存在于子状态机中**，因为退出子状态机后，需要返回到父状态机继续执行下一个过渡；若退出顶级状态机，则动画流程终止，角色静止不动，而在一般情况下，角色是在持续做运动的。<br>若需要让角色完全静止不动，可以通过关闭当前动画图所在的动画控制器组件来实现。
...
右键点击 [动画图面板](animation-graph-panel.md) 右侧的网格布局区域，即可在菜单中选择添加状态：
...
- **动画剪辑内容**：用于指定当前状态上挂载的动画剪辑。可直接从 **资源管理器** 中将动画剪辑拖拽到 **Clip** 属性框中，也可以直接点击 **Clip** 属性框后面的箭头按钮选择。设置了动画剪辑后，当前状态将会播放指定的动画剪辑内容。
...
- 点击齿轮图标按钮，展开的菜单中包括 **删除此状态**、**添加状态组件**、**重命名**。
