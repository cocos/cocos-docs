...
一般情况下，一个对象拥有多个状态，并按照一定的逻辑顺序在状态之间切换，以执行不同的动作，这种切换在动画图中称为 [过渡](state-translation.md)。发生过渡时需要满足的条件称为 **过渡条件**。
...
状态机中的状态除了用于控制动画播放的状态，还包括了 **Entry**、**Exit**、**Any**、**子状态机** 这几个比较特殊的，因为其本身并不承载动画，只是为了标志状态机开始、结束等特殊用途，因此为了与状态区分我们暂且称之为“伪状态”。
...
Entry、Exit、Any 则是默认存在于状态机中的基础单元，不可删除。在打开动画图编辑状态机时便可在网格布局区域中看到，需要注意的是 Exit 只存在于 **子状态机** 中。

![animation-graph-panel](animation-graph-panel/animation-graph-panel.png)

- **Entry**：标志着状态机的进入。Entry 只能作为状态过渡发生的源头，无法作为过渡的目标。

- **Exit**：当状态过渡到 Exit 后，退出状态机。Exit 只能作为状态过渡的目标，无法作为过渡的源头。

    **Exit 仅存在于子状态机中**，因为退出子状态机后，需要返回到父状态机继续执行下一个过渡；若退出顶级状态机，则动画流程终止，角色静止不动，而在一般情况下，角色是在持续做运动的。<br>若需要让角色完全静止不动，可以通过关闭当前动画图所在的动画控制器组件来实现。

- **Any**：可指代状态机中其它任何状态，但不包括“伪状态”。**Any** 只能作为状态过渡的源头，无法作为过渡的目标。

    从 **Any** 中引出过渡等价于从所有状态都引出了该过渡，也就是说，状态机中所有状态在满足某个条件时都会切换到另一个状态，就可以使用 **Any**。例如角色在走、跑、跳等状态下触发攻击条件，都需要切换到攻击状态。

    > **注意**：**Any** 是可以继承的，若在父状态机中设置了从 **Any** 中引出状态过渡，则其子状态机中所有的状态若满足条件都会执行该过渡。
...
右键点击 [动画图面板](animation-graph-panel.md) 右侧的网格布局区域，即可在菜单中选择创建 **状态**（Clip Motion）、**一维动画混合状态**（Blend 1D）、**二维动画混合状态**（Blend 2D），以及 **子状态机**（State Machine），除子状态机外，其他的都以蓝色图形显示在网格区域中。

![Edit](animation-controller/edit.png)

动画混合状态是由多个状态混合而来，详情请参考下文 **动画混合** 部分的内容。
...
> 这里是以状态为例展开介绍，动画混合状态和子状态机可设置的属性会有所差异，具体请参考下文相应的内容介绍。

- 当前选中状态的名称会以黄色字体显示在 **属性** 分页中，例如上图中的 `Clip Motion`，点击右侧的齿轮图标按钮可重命名。

- Speed：用于设置当前状态上挂载的动画剪辑的播放速度，默认为 1，值越小播放速度越慢。

- 动画剪辑内容：用于指定当前状态上挂载的动画剪辑。可直接从 **资源管理器** 中将动画剪辑拖拽到 **Clip** 属性框中，也可以直接点击 **Clip** 属性框后面的箭头按钮选择。

  > **注意**：若未指定动画剪辑，将会导致问题<br>

点击右侧的齿轮图标菜单，可删除当前动画剪辑，然后根据需要重新选择添加动画剪辑内容/一维动画混合/二维动画混合。
...
- 点击状态名右侧的齿轮图标菜单，可以看到支持 **删除此状态**/**添加状态组件**/**重命名**/**替换为动画剪辑**/**替换为一维动画混合**/**替换为二维动画混合**，其中 **添加状态组件** 和 **删除此状态** 也可以直接在网格布局区域中右键点击状态进行操作。
...
- 一维混合：根据一个输入参数通过线性插值算法对动画进行混合。

- 二维混合：根据两个输入参数通过重心坐标插值混合算法对动画进行混合。

动画混合跟 [状态过渡](state-translation.md) 的区别在于：
...
右键点击动画图面板的网格布局区域即可选择添加 **一维动画混合状态**（Blend 1D） 或者 **二维动画混合状态**（Blend 2D）。创建完成后在 **属性** 分页中即可设置属性，除了动画混合设置项，其他项与上文 **设置状态属性** 中的一致。

![blend edit](animation-controller/blend-edit.png)

进入动画混合状态的编辑页面可以设置相关参数及参与混合的动画，进入方式包括以下三种方式：
- 点击动画混合设置项中的 **编辑**
- 双击网格布局中的混合状态
- 右键点击网格布局中的混合状态，然后选择 **编辑**

面板上方的 **位置** 表示当前所处位置，点击层级即可返回相应位置。
...
- 当输入参数位于两个阈值之间，将根据输入参数在阈值区间上的比例 **线性** 混合这两个动画。线性公式为 **A * (1 - t) + B * t**，其中 A、B 表示阈值，t 表示输入参数。

进入动画混合状态编辑页面，在网格布局区域点击右键，即可根据需要添加参与混合的动画，支持添加 **动画剪辑**、**一维动画混合**、**二维动画混合**。

![add-motion-of-blend](animation-controller/add-motion-of-blend1.png)

- **入参**：该项用于选择并设置动画混合的输入参数，目前支持使用 **浮点型** 变量，调节范围为下方设置的阈值的区间。输入参数可在动画图面板的 **变量** 分页中创建，详情请参考 [动画图面板 — 变量](animation-graph-panel.md)。<br>该项也可以直接在网格布局区域的动画混合状态中设置。

- **阈值**：当动画混合状态上添加了动画后该项才可见。每添加一个动画，下方便会新增一个相对应的阈值，可根据需要手动设置每个阈值大小；也可以勾选 **自动调整阈值** 自动均分各个阈值，二者选其一。所有的阈值按从小到大的顺序从上到下排列。

- 可直接在网格布局区域的动画上挂载动画剪辑，也可以选中动画后在左侧的 **属性** 分页中指定动画剪辑。

  ![choose clip](animation-controller/choose-clip.png)

  若需要移除动画，则直接在网格布局区域中右键点击动画以删除，也可以选中动画后在左侧的 **属性** 分页中点击齿轮图标按钮以删除。

例如要实现根据角色速率来均匀混合待机、走、跑的动画，可设置为下图：
...
二维动画混合状态的基本操作与一维动画混合基本一致，区别在于：
- 二维动画混合在 **属性** 分页上的 **入参** 有两个；
- 阈值是以 `[x, y]` 表示，按照动画的添加顺序自上而下排列。并且没有 **自动调整阈值** 选项。
- 第一个输入参数的调节范围为下方阈值 `X` 的区间，第二个输入参数的调节范围为下方阈值 `Y` 的区间。

二维动画混合状态经常用于插值混合不同方向上的移动动画，例如：
...
因为对象一般会有多个阶段的复杂动作组合，为了降解复杂的逻辑，还可以在状态机中无限嵌套 **子状态机**。一般我们将动画图最上层的状态机称为 **顶级状态机**，类似于 **层级管理器** 中的根节点。
...
- ① 号按钮用于进入子状态机编辑页面；
- ② 号按钮用户添加状态组件，详情请参考下文 **状态组件** 部分的内容；
- 点击齿轮图标按钮，展开的菜单中包括 **添加状态组件**、**重命名**、**删除**。

以上功能点大多也可以通过右键点击网格布局区域中的子状态机来实现。

#### 编辑子状态机

进入子状态机编辑页面后，界面如下图所示。点击界面上方的 **位置** 即可返回父状态机。

![edit sub state](animation-controller/edit-sub-state.png)

子状态机的编辑与顶级状态机唯一的区别在于，子状态机中默认包含 **Exit** 伪状态，当状态过渡到 **Exit** 后，退出子状态机，返回到父状态机继续执行下一个过渡，直到回到顶级状态机。详情请参考上文 **状态机基础** 部分的内容。
