...
当 Texture 的原始大小与屏幕映射的纹理图像不一致时，通过不同的纹理过滤方式进行纹理单元到像素的映射会产生不同的效果。Texture 支持三种过滤方式。
...
除了在编辑器中直接设置图像资源的过滤方式，引擎中也提供了`cc.view.enableAntiAlias`接口去动态设置 Texture 是否开启抗锯齿功能，如果开启了抗锯齿，那么游戏中所有 Texture 的过滤方式都将使用线性过滤，否则将使用邻近点采样的过滤方式。
