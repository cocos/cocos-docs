...
ETC1 和 PVR 格式都会用一个固定的空间来存储一个像素的颜色值，当需要存储 rgba 4 个通道时，图片的显示质量可能会变得非常低。所以额外提供了一个 separate alpha 选项，该选项会将贴图的 alpha 通道提取出来合并到贴图下方，然后整张贴图按照 rgb 3 个通道的格式来压缩，每个通道的存储空间得到了提升，贴图的质量也就提升了。
