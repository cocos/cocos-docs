...
虽然 **我们不建议开发中的项目，特别是即将上线的项目强升 3.0**，但是我们仍将在 3.0 正式版推出 2.x 资源导入工具。此工具支持旧项目资源完美导入，以及代码的辅助导入。代码辅助导入会把 JavaScript 转换成 TypeScript，并自动添加组件类型声明、属性声明及函数声明，组件在场景中的引用都会得到保留，并且函数内部的代码会以注释的形式导入，减轻开发者的升级难度。
...
loader API 与 Creator v2.4 一致，可以参考 [v2.4 资源管理模块升级指南](https://docs.cocos.com/creator/manual/zh/release-notes/asset-manager-upgrade-guide.html)
...
Creator v2.4.3 构建 Web Desktop 平台后生成的目录：

![image](web-v243.png)

Creator 3.0 构建 Web Desktop 平台后生成的目录：

![image](web-v3.png)

从以上两张图可以看出，Web 平台 Creator v2.4.3 构建出来的内容与 Creator 3.0 大部分是相同的，不同之处包括以下几点：

1. Creator 3.0 将引擎相关的代码，例如核心模块、物理模块、插件脚本等都统一放到了 **cocos-js** 目录下，相比 Creator v2.4.3 分散放在构建目录中看起来更加清晰。

    ![image](web-cocosjs.png)

2. Creator v2.4.3 只有一个启动脚本 `main.js`，而 Creator 3.0 则有以下两个启动脚本：
    - `index.js`：用于做一些预处理工作
    - `application.js`：用于启动游戏

3. Creator v2.4.3 中用于管理配置的 `src/settings.js` 在 Creator 3.0 改为 `src/settings.json`。

4. Creator v2.4.3 中的首屏图片 `splash.png`，在 Creator 3.0 则默认存储在 `settings.json` 中。

5. Creator v2.4.3 中的 `style-desktop.css` 和 `style-mobile.css`，在 Creator 3.0 则合并为 `style.css`。
...
从以上两张图可以看出，微信小游戏平台 Creator v2.4.3 构建出来的内容与 Creator 3.0 大部分是相同的，不同之处包括以下几点：

1. Creator 3.0 将引擎相关的代码，例如核心模块、物理模块、插件脚本等都统一放到了 `wechatgame/cocos-js` 目录下。而 Creator v2.4.3 则分散一部分放在 `wechatgame` 目录下，一部分放在 `wechatgame/cocos` 目录下。

    ![image](wechat-cocosjs.png)

2. Creator v2.4.3 将小游戏的适配层代码都编译到 `adapter-min.js` 中，而 Creator 3.0 则是将适配层代码全部以散文件的形式存储在 `libs` 目录下，没有进行编译。

3. Creator v2.4.3 的启动脚本是 `main.js`，Creator 3.0 的启动脚本则是 `application.js`。

4. Creator v2.4.3 将所有动态代码的引用记录在了 `ccRequire.js` 中，而 Creator 3.0 目前暂时没有这个功能。

5. Creator v2.4.3 中用于管理配置的 `src/settings.js` 在 Creator 3.0 改为 `src/settings.json`。
...
从以上两张图可以看出，Windows 平台 Creator v2.4.3 构建出来的目录与 Creator 3.0 差异较大。<br>
因为各个原生平台（例如 Android、Windows）构建后生成的底层 C++ 代码是完全一致的，所以在 Creator 3.0，我们将 Creator v2.4.3 存放在构建目录 `frameworks/runtime-src/Classes` 中的底层 C++ 代码单独提取出来，根据构建 **模板** 放在共享的 `common-link`/`common-default` 文件夹中。这样在构建原生平台时，如果检测到已经存在 `common-link`/`common-default` 文件夹，这部分内容便不会再进行处理，加快构建速度。

而 `Windows` 文件夹则是 Creator 3.0 用于存储当前构建的原生平台相关的内容（如果构建的是 Android 平台则生成 `android` 文件夹）：
...
1. Creator v2.4.3 构建目录中属于应用层的文件在 Creator 3.0 都统一合并到了 `assets` 目录中。应用层的文件包括：
    ...
    Creator 3.0 `assets` 目录结构如下：

    ![image](v3-assets.png)

    Creator 3.0 在合并的过程中也做了相应的调整和改动：

    - 将原先 Creator v2.4.3 全部放在构建目录 `src/cocos2d-jsb.js` 文件中引擎相关的代码（例如核心模块、物理模块、插件脚本等）都放到了构建目录 `assets/src/cocos-js` 目录下。

      ![image](windows-cocosjs.png)

    - Creator v2.4.3 只有一个启动脚本 `main.js`，而 Creator 3.0 除了 `main.js` 之外还在 `src` 目录下新增了一个启动脚本 `application.js`，用于启动游戏。

    - Creator v2.4.3 中用于管理配置的 `src/settings.js` 在 Creator 3.0 改为 `assets/src/settings.json`。

2. Creator v2.4.3 将所有原生平台的构建模板都生成在 `frameworks/runtime-src` 目录下：

    ![image](v243-build-template.png)

    而 Creator 3.0 则是将构建模板生成在 `build` 目录下，且只生成当前构建平台的构建模板：

    ![image](v3-build-template.png)

3. 一些编译时需要用到的资源，例如应用图标、应用启动脚本等，Creator v2.4.3 是存储在构建模板中，而 Creator 3.0 则是存储在构建目录的 `windows/proj` 目录下。
