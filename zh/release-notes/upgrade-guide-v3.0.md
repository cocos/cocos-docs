...
- 节点上 UI 相关接口变更如下：
    ...
    - 与坐标变换计算相关的接口如下：
        - 属性：`width`、`height`、`anchorX`、`anchorY`、`anchorPoint`、`contentSize`。
        - 方法：`setAnchorPoint`、`setContentSize`、`convertToNodeSpaceAR`、`getBoundingBox`、`getBoundingBoxTo`。需要先获取节点上的 `UITransform` 组件，再使用对应的接口，例如：
    ...
    - 其余接口如下：
        ...
        - `opacity`：如果节点上有渲染组件，就直接设置渲染组件的 `color`。如果没有渲染组件，则可以通过添加 `UIOpacity` 组件，并设置相关属性。
        - `skew`：该接口已被移除。
        - `group`：变更为 `layer`。
- `CCSpriteFrame`：`copyWithZone`、`copy`、`clone` 和 `ensureLoadTexture` 被废弃。`setFlipX` 和 `isFlipX` -> `flipUVX`、`setFlipY` 和 `isFlipY` -> `flipUVY`、`getTexture` 和 `setTexture` -> `texture`（此处的类型是 Texture2D/RenderTexture）等接口发生变动。其余 `get` 和 `set` 对应的方法（例如：`getOffset`）在 3.0 中都直接对应同名属性（例如：`offset`）。
- `CCTexture2D`：接口发生变动的有 `genMipmaps` -> `mipmaps`、`initWithElement` -> `image`、`initWithData` 整个方法被移除，类似的使用是将原先要传入的 `ArrayBufferView` 数据，传给新建的 `ImageAsset`，然后再用 `ImageAsset` 传给新建的 `Texture2D`，从而获得一份图片资源。
- `cc.Action`: 相关全部移除。
- **物理**：`cc.Collider` -> `Collider2D`、`cc.BoxCollider` -> `BoxCollider2D`、`cc.RigidBody` -> `RigidBody2D` 等。`cc.Collider3D` -> `Collider`、`cc.BoxCollider3D` -> `BoxCollider`、`cc.RigidBody3D` -> `RigidBody` 等。
- **tween**：`cc.repeatForever`、`cc.reverseTime`、`cc.show` 等，对应到 3.0 为 `Tween.repeatForever`、`Tween.reverseTime`、`Tween.show` 等。
- **动画**：`addClip` -> `createState`、`getClips` -> `clips`、`playAdditive` -> `crossFade`、`getAnimationState` -> `getState` 等。
- **相机**：`findCamera`、`alignWithScreen`、`main`、`cameras`、 `zoomRatio` 和 `containsNode` 被废弃。其余变动，例如：`backgroundColor` -> `clearColor`、`cullingMask` -> `visibility`、`depth` -> `clearDepth`、`getScreenToWorldPoint` -> `screenToWorld`、`getWorldToScreenPoint` -> `worldToScreen`、`getRay` -> `screenPointToRay` 等。
- **音频**：`getLoop` 和 `setLoop` -> `loop`、`getVolume` 和 `setVolume` -> `volume`、`getCurrentTime` 和 `setCurrentTime` -> `currentTime`、`src` -> `clip` 等接口变更。
- **材质**：所有相关改动都需要获得 **MeshRenderer** 或其子类身上的 **材质实例** 来完成。`setBlend`、`setDepth`、`setStencilEnabled`、`setStencil` 和 `setCullMode` 被废弃，调用 `overridePipelineStates` 完成更新。`define` 被废弃，调用 `recompileShaders` 完成更新。
- **sys** 下的平台变量变更如下：
