...
| 属性 | 说明 |
| :-- | :-- |
| **ConnectedBody** | 关节链接的另一端的刚体 <br> **注意**：在 v3.7 版本中，如该属性没有被设置，则会默认取原点坐标，以与 **3D 物理** 中的约束在实现上统一 <br> 由于目前 2D 世界的坐标原点默认取 Canvas 所在的左下角，此时关节的 **ConnectedBody** 会连接到左下角。如果觉得这样对开发不太方便，可以通过将 Canvas 进行偏移的方式来处理; 而由于 Canvas 组件本身无法修改其位置信息，可以在通过给 Canvas 组件所在的节点添加父节点并偏移父节点来解决 |
| **Anchor** | 关节本端链接的刚体的锚点 |
