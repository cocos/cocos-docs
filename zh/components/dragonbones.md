...
| Animation          | 当前播放的动画名称
| Render Mode        | 渲染模式，默认realtime模式。<br>realtime模式，实时运算，支持dragonbones所有的功能。<br>sharedCache模式，将骨骼动画数据进行缓存并共享，相当于将骨骼动画转换为帧动画，拥有较高性能，较少内存占用，不支持动作融合，只支持动作开始和结束事件，适用于特效，npc动画等。<br>privateCache模式，与sharedCache类似，但不共享动画数据。
