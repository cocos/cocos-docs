...
```js
    // 获取下载进度回调
    cc.assetManager.loadAny({ 'path': 'image/background' }, { onProgress: function (loaded, total) {
...
```js
    cc.assetManager.loadAny({ path: 'images/background', type: cc.SpriteFrame }, callback);
```

这种方式完全等价于直接使用 `cc.resources.load`。
...
```js
    ...
    cc.assetManager.loadAny({ url: 'http://example.com/test.myformat' }, callback);
```
