...
### 面板名称或其他重要界面元素，用加粗表示，并且相邻的正文之间需要加空格
...
### 编辑器中的属性名按照属性检查器中显示的格式书写，用加粗表示，并且和相邻的正文之间加空格

> 格式：拖拽任意一张图片资源到 \*\*Sprite Frame\*\* 属性中
...
### 脚本属性和方法名按照 API 中显示的格式书写，用 backtick 表示，并且和相邻的正文之间加空格
...
### 添加图片，同样的与上下正文之间需要使用空行隔开，并且和相邻的正文之间需要加空格

图片格式：**![图片说明，可不填]\(图片的相对路径)**。使用半角（halfwidth）英文标点，且 !、[]、() 三者之间不要有空格
...
> e.g：[Mask API]\(../../../api/zh/classes/Mask.html)。跨文档的文件名后缀要使用 **.html**

### 换行使用空行或者 \<br\>，如果使用回车键换行在 GitBook 上没有换行效果并且会增加一个空格
...
当正文使用 - 分点介绍时，分点中的正文包括图片都需要保留 2/4 个空格的缩进，一般使用两个空格的缩进。例如：
...
当正文使用 **数字** 分点介绍时，分点中的正文包括图片都需要保留 **4** 个空格的缩进。例如：
