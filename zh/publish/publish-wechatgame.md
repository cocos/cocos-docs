...
- 桌面端下载 [微信开发者工具](https://mp.weixin.qq.com/debug/wxagame/dev/devtools/download.html) 并安装。
...
在微信小游戏环境中，资源管理是最特殊的部分，它和浏览器的不同包括以下四点：
...
4. 如果找不到就从远程服务器下载资源
5. 资源下载到临时文件后直接使用
6. 后台缓慢地将资源从临时文件保存到小游戏应用缓存内，以便再次访问时使用
...
当微信小游戏升级引擎版本后，留在本地的缓存资源还是之前旧版本引擎对应的资源，并不会自动清空。这可能会导致资源加载出错或渲染错误等问题。解决方案有以下几种：
...
构建完成后，初始场景会被构建到发布包目录下的 `assets/start-scene` 文件夹，也就是 bundle 中。该 bundle 不会放在远程服务器上，而是放在本地，引擎在启动阶段时就会自动从本地包内加载这个 bundle，从而加快初始场景的加载速度。
