...
一、打开主域项目，在 **菜单栏 -> 项目** 中打开  **构建发布** 面板，选择 **微信小游戏** 平台，填入 **开放数据域代码目录**。该目录是开放数据域构建后所在的路径，并且这个路径需要放在主域构建目录下。然后点击 **构建**。
...
二、打开开放数据域项目，打开 **构建发布** 面板，选择 **微信小游戏开放数据域** 平台。
...
**注意：如果先构建开放数据域再构建主域，那么开放数据域的发布代码会被覆盖。我们已在 v2.0.7 版本中修复该问题**
...
Cocos Creator 提供的 [微信小游戏开放数据域范例工程](https://github.com/cocos-creator/demo-wechat-subdomain/archive/1.x.zip) 包含了微信小游戏开放数据域的使用示例。
