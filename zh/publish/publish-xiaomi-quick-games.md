...
分包 rpk 是根据用户的需求选择是否使用。分包加载，即把游戏内容按一定规则拆分在几个包里，在首次启动的时候只下载必要的包，这个必要的包称为 **主包**，开发者可以在主包内触发下载其他子包，这样可以有效降低首次启动的消耗时间。若要使用该功能需要在 Creator 中设置 [分包加载](../scripting/asset-bundle.md)，设置完成后在构建时就会自动分包。构建完成后，会生成 **.rpk** 文件，生成目录在 **build/xiaomi/dist** 目录下。
