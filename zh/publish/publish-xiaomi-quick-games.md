...
- PC 端全局安装 npm-5.2.0 或以上版本。升级 npm 命令如下：
...
- **支持的最小平台版本号**
  ...
  该项为必填项。根据小米的要求目前这个值必须大于或等于 **1041**。
...
- **密钥库**
  ...
  如果不勾选 **密钥库**，则需要配置签名文件 **certificate.pem 路径** 和 **private.pem 路径**，此时构建出的是可以 **直接发布** 的 rpk 包。用户可通过输入框右边的 **...** 按钮来配置两个签名文件。**注意**：这两个签名文件需要指定到发布包 `\build\xiaomi\sign\release` 目录下。
...
分包 rpk 是根据用户的需求选择是否使用。分包加载，即把游戏内容按一定规则拆分在几个包里，在首次启动的时候只下载必要的包，这个必要的包称为 **主包**，开发者可以在主包内触发下载其他子包，这样可以有效降低首次启动的消耗时间。若要使用该功能需要在 Creator 中设置 [分包加载](../scripting/subpackage.md)，设置完成后在构建时就会自动分包。**注意**：单个分包/主包大小不能超过 **4M**。
