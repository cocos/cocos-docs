...
- **小包模式**
  ...
  3. 构建完成后，点击 **发布路径** 后面的 **打开** 按钮，将发布路径下的 **/quickgame/remote** 目录上传到小包模式服务器。例如：默认发布路径是 build，则需要上传 /build/quickgame/remote 目录。

  此时，构建出来的 rpk 将不再包含 remote 目录，remote 目录里的资源将通过网络请求从填写的 **小包模式服务器地址** 上下载。
