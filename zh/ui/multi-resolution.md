...
如果对于屏幕周围可能被剪裁的内容没有严格要求，可以同时勾选 Canvas 组件的 `Fit Width` 和 `Fix Height`，这样子相当于开启了 SHOWALL 模式，此时会根据屏幕宽高比自动适配高度和宽度来避免黑边。也就是说，设计分辨率宽高比大于屏幕分辨率时，会自动适配高度（上面第一张图）；设计分辨率宽高比小于屏幕分辨率时，会自动适配宽度（上面第二张图）。
