...
目前 Cocos Creator 支持硬采样、4 倍(**SOFT** 模式)、9 倍(**SOFT_2X** 模式)采样，倍数越大，采样区域越大，阴影边缘也就越模糊。

#### AutoAdapt 自适应阴影计算技术

AutoAdapt 自适应阴影计算会自动计算视口（lightView）下阴影产生的范围，以及阴影相机的远近。可用于检测硬件是否支持半浮点类型，若支持则 Creator 会自动开启线性深度及使用纹理压缩。
