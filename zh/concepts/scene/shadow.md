...
当项目中存在大批量相同物体（或者对象？贴图？），但由于内存原因无法进行合批，就会导致渲染时间过长。这时候可以保存这些静态物体（例如植被等）的位置、缩放和 uv 偏移等相关信息，然后将其全部从场景中剔除，在需要渲染的时候再根据保存的信息，通过 Instance 来渲染。
