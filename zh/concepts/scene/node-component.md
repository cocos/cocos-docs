...
从 3.0 版本以后，引擎采用根据相机的 Visibility 属性跟节点的 layer 属性，来控制其可见性，不再采用 Canvas 及其节点控制 2D 组件的可见性，使用更加通用的节点与相机匹配来控制可见性。好处是更灵活的控制组件的可见性，使分组显示多样化。同时 3D 组件与 2D 组件可以混合渲染。
