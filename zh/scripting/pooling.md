...
这会产生一点点的性能问题。虽然通常情况下这并不致命，但是在做大批量、频繁的操作对象池时（比如制作射击游戏弹幕），可能会在中低端机器上引起可感知的性能问题。
除了性能问题，不断的·removeFromParend·和·addChild·操作也会导致对象对象的默认渲染顺序发生变化，为了避免这种情况，建议用`setSiblingIndex`指定对象的索引。
