...
但如果你想要将相关版本配置信息存储在服务器上，启动时动态获取版本信息以实现热更新，你也可以手动指定一个版本 Hash 值并传入 `loadBundle` 中，此时将会以该传入的 Hash 值为准：
...
Asset Bundle 提供了 `loadScene` 方法用于加载 Asset Bundle 中的场景，你只需要传入 **场景名** 即可。<br>
`loadScene` 与 `cc.director.loadScene` 不同的地方在于 `loadScene` 只会加载 Asset Bundle 中的场景，而不会运行场景，你还需要使用 `cc.director.runScene` 来运行场景。
...
当 Asset Bundle 被加载过之后，会被缓存下来，此时开发者可以使用 Asset Bundle 名称来获取该 Asset Bundle。例如：
...
3. 使用 Asset Bundle 提供的 `releaseAll` 方法，此方法与 `cc.assetManager.releaseAll` 相似，`releaseAll` 方法会释放所有属于该 Asset Bundle 的资源（包括在 Asset Bundle 中的资源以及其外部的相关依赖资源），请慎重使用。
...
在加载了 Asset Bundle 之后，该 Asset Bundle 会一直存在整个游戏过程中，除非开发者手动销毁。当手动销毁了某个不需要的 Asset Bundle，那么该 Asset Bundle 的缓存也会被移除，如果需要再次使用，则必须再重新加载一次。
...
**注意**：在销毁 Asset Bundle 时，并不会释放该 Asset Bundle 中被加载过的资源。如果需要释放，请先使用 Asset Bundle 的 `release` / `releaseAll` 方法：
...
释放资源相关可参考上一个部分 [释放 Asset Bundle 中的资源](../scripting/asset-bundle.md#%E9%87%8A%E6%94%BE-asset-bundle-%E4%B8%AD%E7%9A%84%E8%B5%84%E6%BA%90) 的内容介绍。
