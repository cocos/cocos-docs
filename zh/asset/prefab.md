...
### 删除组件
在预制件实例下删除的组件，会在 **属性检查器** 上增加一条删除的组件的数据，它的数据存储在预制件的实例下，所以不会影响关联的预制件资源的数据。
...
图标|描述
:---|:---
![](prefab/revert-remove-component.png)|还原删除的这个组件
![](prefab/apply-remove-component.png)|应用删除这个组件的操作到关联的预制件资源中
