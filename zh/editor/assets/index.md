...
资源生成分为两种，一种是通过编辑器创建的方式生成，另一种是通过外部资源导入的形式生成。
...
外部资源导入操作步骤如下：
1. 选中需要导入的资源，拖入到 `assets` 文件管理器处，观察资源是否存在转圈现象，如果没有出现，则代表资源已经导入完成。
2. 右击 `assets` 文件管理器，打开 `assets` 文件管理器所在文件夹，将需要导入资源拷贝到文件夹下，回到编辑器界面，观察资源是否存在转圈现象，如果没有出现，则代表资源已经导入完成。
