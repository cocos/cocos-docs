...
> **注意**：扩展会自动把资源打成一个 zip 包，该 zip 包只适用于 **导入资源包** 扩展使用。
...
自 3.8.3 起，编辑器允许新建的资源类型支持在项目目录下的  `templates/new-asset` 目录下放置资源模板。项目可以根据需要定制自己项目的一些场景、脚本模板，放置的模板会在创建时拷贝到目标目录。

其中不同的资源类型需要以 `importer` 的名字创建文件夹后放置对应的模板文件，`importer` 的名字可以参考资源 meta 文件内的 `importer` 字段，也可以参考内置资源模板的放置目录结构。

![internal-templates](img/internal-templates.png)

放置资源文件的名称将会作为资源创建菜单处的显示名称显示在原始资源创建菜单的二级目录下，编辑器原始内置的默认模板将会命名为 `Default` 显示在最上方。

- 例如：
在 `templates/new-asset/scene/template.scene` 放置的文件后，新建场景资源的创建菜单的二级目录下会出现 `template` 的创建菜单。

    ![new-asset](img/new-asset.png)

项目内如果是以 default 命名的模板，将会覆盖编辑器内置的模板不会新增二级菜单。

## 增强资源管理器面板

目前支持的扩展包括 **右击菜单** 和 **拖入识别**，详情请参考 [增强资源管理器面板](./extension.md)。
