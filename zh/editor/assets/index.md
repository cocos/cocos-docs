...
**资源管理器** 面板支持使用快捷方式来操作资源，详情请参考 [快捷键使用方式](../shortcuts/index.md)。

### 创建资源

资源的创建有两种方式，一种是通过编辑器，另一种是通过导入外部资源。
...
通过导入外部资源的方式创建资源包括以下两种方式：
1. 在文件夹中选中需要导入的资源，拖拽到编辑器 **资源管理器** 面板的 `assets` 目录中，若面板右下角依次显示资源名称且资源显示在面板列表中，则代表资源已经导入完成。
2. 在 **资源管理器** 面板中右键点击 `assets` 目录，选择 **在资源管理器中显示**，打开 `assets` 目录所在的文件夹，将需要导入的资源拷贝到 `assets` 文件夹下。回到编辑器界面，观察资源是否存在转圈现象，有的话，待转圈结束即表示资源导入完成。若导入的资源较小没有出现转圈现象，则面板右下角依次显示资源名称且资源显示在面板列表中，即代表资源已经导入完成。
...
- **限定搜索类型**：可多选。过滤类型为 [资源类型](__APIDOC__/zh/modules/asset.html)。
...
- **指定搜索字段**：搜索名称不区分大小写，包括以下几种搜索方式：
    ...
    3. **搜索 URL**
        ...
        Cocos Creator 对资源路径也做了一层类似盘符的封装，资源库（dataBase）的盘符路径为 `db`。意味着 **资源管理器** 内的所有资源搜索都从 `db` 开始，即使 `assets` 和 `internal` 资源库在实际的操作系统下，它们路径的父级并不相同。
        - 假设需要搜索的资源 `hero.prefab` 存放在 `assets` 下的 `model` 文件夹里，则可以通过 `db://assets/model/hero.prefab` 检索到。
        - 假设需要搜索的资源 `ui-sprite-material.mtl` 存放在 `internal` 下，则可以通过 `db://internal/ui-sprite-material.mtl` 检索到。

    4. **查找 UUID 的使用**，例如：

        ![search-uuid](img/search-uuid.png)

若只想在某个父级资源（例如：文件夹）中搜索，则右键点击并选择 **在文件夹中查找**，即可缩小搜索范围。
...
选中具体的资源/文件夹后，点击鼠标右键，可以在弹出的菜单栏中对资源执行一系列操作：
...
- **UUID/URL/PATH**：复制资源的 UUID/URL/PATH，并且在 **控制台** 面板中输出。此处输出的信息可用于资源搜索处的 **指定搜索字段**。
