...
| 保存场景 | 保存当前正在编辑的场景，如果是使用 **文件 -> 新建场景** 创建的场景，在第一次保存时会弹出对话框，选择场景文件保存的位置和文件名。场景文件以 `.fire` 作为扩展名 |
...
| 选项 | 说明 | 默认快捷键 |
| :--- | :-- | :--|
| 撤销 | 撤销上一次对场景的修改 |  Ctrl/Command + Z
| 重做 | 恢复上一步骤的撤销动作 |  Ctrl/Command + Shift + Z
| 剪切 | 剪切当前选中的节点或字符到剪贴板| Ctrl/Command + X
| 复制 | 复制当前选中的节点或字符到剪贴板 | Ctrl/Command + C
| 粘贴 | 粘贴剪贴板中的内容到场景或属性输入框中| Ctrl/Command + V
| 全选 | 焦点在场景编辑器内为选中所有节点，焦点在控制台则选中所有的日志信息 | Ctrl/Command + A

> **注意**：节点的 **复制**、**剪切** 和 **粘贴** 请通过快捷键操作。
...
| 功能 | 说明 |
| :--- | :-- |
| 视角对齐 | 将当前选中节点移动到 **场景编辑器** 的视角中心 |
| 将视角和节点对齐 | 将 **场景编辑器** 中的视角设置为以当前选中节点为中心 |
| 取消关联当前的 [Prefab](../../asset/prefab.md) 资源 | 选中场景中一个预制节点，执行此命令会将预制节点转化成普通节点|
| 递归的取消关联当前的 [Prefab](../../asset/prefab.md) 资源 | 用于将场景中包含嵌套的预制节点递归地转化为普通节点 |
| 空节点 | 在场景中创建一个空节点，如果执行命令前场景中已经选中了节点，新建的节点会成为选中节点的子节点|
| 3D 对象 | Creator 提供了一些比较基础的静态模型控件以便使用，目前包括立方体、圆柱体、球体、胶囊、圆锥体、圆环体、平面和四方形。若需要创建其他类型的模型，可参考 [MeshRenderer 组件](../engine/../../engine/renderable/model-component.md)|
...
| 摄像机 | 在场景中创建 Creator 预设好的包含 Camera 组件的节点。具体的使用方式，详情请参考 [摄像机组件](../components/camera-component.md)|
...
| 运行预览 | 点击该项即可在选择的浏览器/模拟器/预览窗口中预览项目效果，详情请参考 [预览调试](../preview/index.md)|
| 刷新预览 | 刷新已经打开的预览窗口  |
| 生成预览模板 | 该项用于自定义想要的预览效果，详情请参考 [自定义预览模板](../.././editor/preview/browser.md)|
| 构建发布 | 打开 [构建发布](../.././editor/publish/index.md) 面板，该面板用于将游戏打包发布到目标平台上 |
| 构建项目构建模板 |  该项用于自定义想要的预览效果，详情请参考 [自定义构建模板](../.././editor/publish/custom-project-build-template.md)|
| 新建构建扩展包 | 该项用于扩展构建流程，详情请参考 [构建扩展包](../.././editor/publish/custom-build-plugin.md)|
...
| 功能 | 说明 |
| :--- | :-- |
| 控制台 |  点击该项即可打开 [控制台](../console/index.md)，用于查看输出的日志信息 |
| 场景编辑器 | 打开 [场景编辑器](../scene/index.md) 面板。该面板用于选择和摆放场景图像、角色、特效、UI 等各类游戏元素 |
| 资源管理器 | 点击该项即可选择打开 **资源管理器/资源预览** 面板，用于访问/管理/查看项目资源，详情请参考 [资源管理器](../assets/index.md) |
| 属性检查器 | 点击该项即可选择打开 [属性检查器](../inspector/index.md) 面板，用于查看并编辑当前选中节点、节点组件和资源的工作区域  |
| 层级管理器 | 点击该项即可选择打开 [层级管理器](../hierarchy/index.md) 面板，该面板用于展现当前场景中可编辑的节点之间的关系。|
| 预览 | 点击该项即可选择打开摄像机预览面板，作为简单的预览功能存在，当调整 gizmo 或通过其它插件调整场景时，摄像机预览的画面可以实时同步 |
| 动画 |  可以打开 [动画编辑器](../.././animation/index.md) 面板或者 [Joint Texture Layout（骨骼贴图布局设置）](../.././/animation/joint-texture-layout.md) 面板，用于对帧动画或者骨骼动画进行编辑、查看等功能。 |
...
| Cocos 小秘书 | 打开 [Cocos 小秘书](https://www.cocos.com/assistant) 面板，与 Cocos Creator 编辑器紧密结合，⽆缝嵌⼊开发环境，⽆论你遇到任何问题，随时可以发起交流。目前仅支持中文版本|
...
| 商城 | 点击该项即可进入 Cocos Creator 内置的 [扩展商城](https://store.cocos.com/app/)，可供用户浏览、下载和自动安装官方或者第三方扩展和资源。同时用户也可以将自己开发的扩展插件、美术素材、音乐音效等资源提交到扩展商店以便分享或者售卖详情请参考 [提交资源到商店](../extension/store/upload-store.md) |
| 创建扩展 | 生成 [扩展包](../extension/readme.md)，用于对编辑器进行功能扩展|
...
| 打开资源调试工具 | 打开资源调试面板，用于修改 asset-db 进程期间的日志消息|
