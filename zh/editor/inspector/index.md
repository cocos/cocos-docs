...
**资源管理器** 面板大致可以分为 **工具栏**、**节点属性设置** 和 **组件属性设置** 几个部分。
...
Prefab 节点在 **属性检查器** 顶部工具栏中的功能包括：取消关联、定位资源、从资源还原、更新到资源。

![prefab-menu](index/prefab-menu.png)

- **取消关联**：是指将 Prefab 节点与 Prefab 资源断开联系，变为普通节点，节点颜色变成白色，不再是绿色。
- **定位资源**：用于在 **资源管理器** 面板上定位到 Prefab 资源，并令其闪烁。
- **从资源还原**：是指将一个已经编辑过组件属性的 Prefab 节点，连同它的子节点，都回退到初始状态。
- **更新到资源**：是指将已经编辑过的 Prefab 节点数据更新到 Prefab 资源。

关于 Prefab 资源，详情可参考 [预制资源（Prefab）](../../asset/prefab.md)。
