...
| **Object Size** | 物体大小，取值范围 [0, 6] <br> 点击右侧的 **Reset Object Size** 按钮会将此属性重设为 1 |
...
点击属性检查器上任意 **> LOD(0 ~ 3)** 后会显示如下图示，其属性及描述如下：

![lod0](lod/lod0.png)

| 属性 | 说明 |
| :-- | :-- |
| **Screen Ratio(%)** | 层级生效时的屏占比，取值范围 [0, 1]。<br> 开发者可以在后方的输入框内输入自定义的屏占比，也可以通过 **Apply Current Screen Ratio** 自动计算屏占比 |
...
| **0 Triangle - 0 Sub Mesh(es)** | 该属性显示了当前层级所配置的三角面的数量以及其子网格的数量 |

### 网格渲染器属性

网格渲染器可以通过点击下方的 ![+](lod/+.png) 以及 ![-](lod/-.png) 按钮进行添加或者删除操作。

添加后可以通过拖拽层级管理器内带有网格渲染器的节点进行配置：

![drag mesh](load/../lod/drag-mesh.gif)

## 预览

配置好 LOD 后，在场景管理器内通过鼠标滚轮即可以预览不同的层级。

![preview](lod/preview.gif)

随着鼠标滚动的变化，LOD 的层级会变化，如当前物体的屏占比低于 LOD2 所配置的屏占比时，场景管理系统会将其剔除（Culled）。屏幕中会显示当前该 LOD Group 的层次状态 LOD(0~3) 或 剔除。
...
请参考 [多层次细节示例](./lod-sample.md) 查看示例。

## 美术工作流

在实际项目开发中，美术工作者可在 DCC 软件（Maya、3D Max、Blender）中进行减面操作，并将模型导出。

通常可以导出多个不同层级的模型或者在同一模型中增加不同的子网格来代表不同的 LOD 层级。
