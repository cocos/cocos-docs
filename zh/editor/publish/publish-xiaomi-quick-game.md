...
分包 rpk 是根据用户的需求选择是否使用。分包加载，即把游戏内容按一定规则拆分在几个包里，在首次启动的时候只下载必要的包，这个必要的包称为 **主包**，开发者可以在主包内触发下载其他子包，这样可以有效降低首次启动的消耗时间。若要使用该功能需要在对应文件属性上勾选分包（参考：[Asset Bundle 压缩类型（分包）](../../asset/bundle.md)），设置完成后在构建时确认分包选项开启后点击构建就会自动分包。**注意**：单个分包/主包大小不能超过 **5M**，所有包体总和不能超过 **10M**。
