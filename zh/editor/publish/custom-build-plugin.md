...
构建插件的注册到构建内入口脚本( builder 字段填写的脚本及其依赖脚本)、自身的 `main` 脚本，将会在主进程载入执行。

当主进程代码修改时，就必须要重启插件之后再去刷新需要更新的进程（这一点后续我们会优化，尽量可以一次重启解决卸载问题，但刷新依旧是最彻底的重载）。主进程目前没有比较比较合适的调试提供，可以使用命令行打开编辑器查看主进程代码 log 来辅助调试。

### 渲染进程（构建面板）

构建插件的入口脚本内，有部分注册到面板里的字段，例如 **`options` 的显示配置、panel 字段及 panel 脚本**，这部分内容会在渲染进程内载入执行。渲染进程类似于浏览器端打开调试工具，可以调试构建面板上的 dom 元素，样式，脚本等等。
