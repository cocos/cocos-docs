...
资源打包过程中会不断地收集参与资源构建的资源 UUID Bundle 的 `config.json` 中，在生成 `config.json` 之前会根据是否为调试模式来决定是否对文件中的 UUID 做压缩处理。
...
3. **资源丢失**
    ...
    - **解决方案**：通过代码编辑器查找该  被哪些资源所引用，修改对应资源。
...
自动图集在构建过程中会打印出原始小图与合成的大图的  信息，在构建调试工具中便可以查找到，然后用查找到的大图的 UUID 在打包后生成的 `XXXBundle/native` 目录中查看即可。如果合图太多，可以打开构建 log 用搜索 UUID 的方式查找。
