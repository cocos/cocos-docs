...
构建主要有两个大流程，一个是 **通用构建处理**，一个是 **平台适配处理**。各个平台的适配处理逻辑将会以构建插件的形式注入到编辑器构建内，每个平台的适配构建都是相互隔离的。平台各自的一些构建参数在构建面板也会以比较特殊的方式显示在界面上，开发者一样可以通过 [构建插件](custom-build-plugin.md) 去动态注入一些构建参数 UI 在界面上方便使用。
