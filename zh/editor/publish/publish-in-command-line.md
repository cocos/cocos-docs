...
- `--build`：指定构建项目使用的参数
  ...
  - `includedModules` - 定制引擎打包功能模块，只打包需要的功能模块。具体由哪些功能模块可以参考引擎仓库下根目录下的 [`cc.config.json`](https://github.com/cocos-creator/engine/blob/3d/cc.config.json) 文件中的 `features` 字段。
