...
- `--build`：指定构建项目使用的参数
  ...
  在 `--build`  后如果没有指定参数，则会使用 Cocos Creator 中 **构建发布** 面板当前的平台、模板等设置来作为默认参数。如果指定了其他参数设置，则会使用指定的参数来覆盖默认参数。可选择的参数有：
    ...
    - `includedModules` - 定制引擎打包模块，只打包需要的模块。定制引擎打包的模块。该参数传入的是模块中的 `entry` 字段，各模块对应的字段可在 [division-config.json](https://github.com/cocos-creator/engine/blob/3d/scripts/module-division/division-config.json) 中查找。
