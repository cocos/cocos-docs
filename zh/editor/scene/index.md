...
| ![scale snap](bar_img/scaling.png)        | 用于设置是否在使用 **缩放变换工具** 时开启自动吸附功能。右侧的方框用于设置缩放步长，默认为 1。 |
...
- 对齐：多个节点在视图内的对齐操作，至少选中需要两个节点才会生效
...
**场景编辑器** 上方的 ![light](bar_img/light.png) 按钮主要用于设置在进行场景编辑时是否使用场景灯光，默认使用。
...
在 3D 视图中，可以通过吸附来进行场景编辑，目前引擎支持两种吸附模式 **顶点吸附** 和 **表面吸附**。

在 **层级管理器** 中选中任意模型后，按下相应的快捷键后，模型下方会出现白色控制块，拖动控制块既可完成吸附功能。
