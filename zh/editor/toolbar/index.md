...
点击主窗口左上角工具栏第四个按钮，或在使用场景编辑器时按下 **T** 快捷键，即可切换使用 **矩形变换工具**。需要注意的是，矩形变换工具只适用于 UI 节点。
...
矩形变换工具激活时：
- 拖拽控制手柄的任一顶点控制点，可以在保持对角顶点位置不变的情况下，同时修改 UI 节点的 **Position** 属性和（`X` 或 `Y` 属性） 和 UITransform 组件中的 **ContentSize** 属性。
- 拖拽控制手柄的任一边控制点，可以在保持对边位置不变的情况下，修改 UI 节点的 **Position** 和 UITransform 组件中的 **ContentSize** 属性（`width` 或 `height` 属性）。
...
- Global：变换工具中手柄箭头的方向以世界坐标系为准，不受节点旋转的影响,如下图：
