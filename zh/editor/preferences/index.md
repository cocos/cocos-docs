...
- **默认 meta**: 这个选项用于控制项目内资源导入的时候的默认配置。

### 默认 meta

例如我们希望导入的图片都默认成为 sprite-frame 类型，那么我们可以先找到 Default Meta 选项，点击 “编辑（Edit）”，这时候会打开一个 json 文件。填入以下内容：
...
其中 "image" 是资源的类型，如果我们想要知道，可以找到资源文件，打开与它对应的 meta 文件，importer 字段里标记的就是资源的类型。

资源类型为 key，value 需要是一个 object，这个 object 就是最终交给导入器使用的默认配置，这个配置与刚刚打开的 meta 文件里的 userData 字段一一对应。这里的 "type" 设置为了 "sprite-frame"，导入的时候，默认的 userData.type 就会被设置为 sprite-frame。这样我们就能够根据项目情况，动态的设置每种资源的默认导入配置。

最后我们需要点击 "应用(apply)" 按钮，刷新资源数据库的配置信息，如果应用，需要等到下次重启程序，才会生效。
