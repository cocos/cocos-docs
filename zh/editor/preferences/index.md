...
- **场景即时缓存**： 用于开启场景即时缓存功能，默认开启。开启后将会在场景编辑的过程中每隔一段时间缓存场景文件到项目目录下的 `temp/scene/[SCENE_UUID]/[TIME].json` 位置，如遇突发情况比如场景奔溃、进程卡死等，下次打开编辑器将会弹窗询问是否应用缓存内最近的一份的场景文件。需要注意，一旦场景正常打开后，在打开之前的缓存文件便会被清理掉。如有特殊需求需要自行翻阅指定的缓存场景文件，请在对应场景关闭的情况下操作。
