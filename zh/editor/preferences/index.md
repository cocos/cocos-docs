...
- **场景即时缓存**： 该项默认开启，主要用于在场景编辑过程中每隔一段时间便将场景文件缓存到项目目录下的 `temp/scene/[SCENE_UUID]/[TIME].json` 文件中。如遇突发情况比如场景崩溃、进程卡死等，再次打开编辑器时将会弹窗提示是否应用缓存内最近一次的场景文件。

**注意**: 只要场景正常打开后，在打开之前的保存的缓存文件便会被清理掉。如有特殊需求需要查看指定的缓存场景文件，请在对应场景关闭的情况下操作。
